    ...
    public class SpacesController : Controller
    {
        ...
        public async Task<IActionResult> Get(string columnName, string columnValue)
        {
            ...
            await using (var connection = new NpgsqlConnection(_databaseService.GetConnectionString("DefaultConnection")))
            {
                ...
                await using (var command = new NpgsqlCommand($"SELECT * FROM \"Стоянка\".\"Spaces\" WHERE {columnName} like '%{columnValue}%';", connection))
                {
                    ...
                    await using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            var spaces = new Spaces
                            {
                                ...
                                Дата_въезда = reader.IsDBNull(2) ? null : await reader.GetFieldValueAsync<DateTime>(2),
                                Госномер = reader.IsDBNull(3) ? null : await reader.GetFieldValueAsync<string>(3),
                            };
